...
Approved retrieval experimentation prospects have been promoted into Phase 1 Epic 5. Session memory is deferred outside Phase 1, and TurboQuant remains exploratory.
...
Status: Approved
Target Stage Or Epic: Phase 1 Epic 5, `RAG-022`–`RAG-031`
Last Reviewed: 2026-08-04
...
Resulting Decision: Promoted into the Epic 5 Colab prototype and experiment runner. Runs must use versioned profiles/manifests, multiple datasets, unique collections, checkpointed Drive artifacts, and evidence-backed merge-back of selected defaults. See the 2026-08-04 Epic 5 decision in `docs/DECISIONS.md`.
...
Resulting Decision: Promoted into Epic 5 through the shared dataset registry, profile schema, and Qdrant experiment runner. Produce use-case guidance rather than one universal model ranking; selected defaults must be captured in reviewed profiles and summary reports.

## Component Laboratory Convention

Approved component experiments run inside the isolated Epic 5 Colab prototype, not as permanent duplicate applications or automatic production tuners. They use configuration-driven profiles, deterministic dataset/query identifiers, unique collection/output names, an explicit baseline, exact run manifests, checkpointed/resumable execution, and repeatable evaluation. Qdrant runs ephemerally on the Colab filesystem. Shared Drive holds datasets and full artifacts; Git receives reviewed code, compact fixtures, selected profiles, manifests, and summaries. Paid services, hosted inference, hosted vector databases, unbounded matrices, and unattended scheduling remain prohibited. A winning run does not silently change product defaults.
